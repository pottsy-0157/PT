...
(function () {
  ...
  function startOfWeek(d){ const date=new Date(d); const diff=date.getDate()-date.getDay(); return new Date(date.setDate(diff)); }
  function formatShort(date){ return `${weekdays[date.getDay()]} ${date.getDate()}`; }
  function combineDateAndTime(dayDate,hhmm){ const [hh,mm]=hhmm.split(":").map(v=>parseInt(v,10)); const d=new Date(dayDate); d.setHours(hh,mm,0,0); return d; }
  function formatTime(d){ return d.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:false}); }
  function ensureNotificationPermission(){ return new Promise(resolve=>{ if(!("Notification" in window)) return resolve(false); if(Notification.permission==="granted") return resolve(true); if(Notification.permission!=="denied"){ Notification.requestPermission().then(perm=>resolve(perm==="granted")); }else resolve(false); }); }
  function scheduleReminder(rem){ const delay=rem.when-Date.now(); if(delay<=0) return; setTimeout(()=>{try{new Notification(rem.name,{body:`Starting at ${rem.time}`});}catch{}},Math.min(delay,2147483647)); }
  function storeReminder(rem){ try{ const list=JSON.parse(localStorage.getItem("reminders")||"[]"); list.push(rem); localStorage.setItem("reminders",JSON.stringify(list)); }catch{} }
  function downloadICS({title, description, start, durationMinutes}){ const end=new Date(start.getTime()+durationMinutes*60000); const pad=n=>n.toString().padStart(2,"0"); const toICSDate=dt=>`${dt.getUTCFullYear()}${pad(dt.getUTCMonth()+1)}${pad(dt.getUTCDate())}T${pad(dt.getUTCHours())}${pad(dt.getUTCMinutes())}00Z`; const escapeICS=s=>String(s).replace(/\\/g,"\\\\").replace(/\n/g,"\\n").replace(/,/g,"\\,").replace(/;/g,"\\;"); const uid=`${start.getTime()}-${Math.random().toString(36).slice(2)}@elev8`; const ics=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//ELEV8//Schedule//EN","BEGIN:VEVENT",`UID:${uid}`,`DTSTAMP:${toICSDate(new Date())}`,`DTSTART:${toICSDate(start)}`,`DTEND:${toICSDate(end)}`,`SUMMARY:${escapeICS(title)}`,`DESCRIPTION:${escapeICS(description)}`,"END:VEVENT","END:VCALENDAR"].join("\r\n"); const blob=new Blob([ics],{type:"text/calendar;charset=utf-8"}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`${title.replace(/\s+/g,"_")}.ics`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }
  ...
  function addSession(container,name,time,dayDate){
    ...
    title.innerHTML=`${time} • ${name}`;
    card.appendChild(title);

    const btnWrapper=document.createElement("div");
    btnWrapper.className="session-btns";

    const btn=document.createElement("button");
    btn.className="remind-btn"; btn.textContent="Remind";
    btn.addEventListener("click",e=>{ e.stopPropagation(); const dt=combineDateAndTime(dayDate,time); ensureNotificationPermission().then(allowed=>{ if(!allowed){ if(toast) toast.textContent="Enable notifications"; setTimeout(()=>{if(toast)toast.textContent=""},2000); return; } const rem={name,time,when:dt.getTime()}; storeReminder(rem); scheduleReminder(rem); if(toast) toast.textContent=`Reminder set for ${name} at ${formatTime(dt)}.`; setTimeout(()=>{if(toast && toast.textContent.includes(name)) toast.textContent=""},2000); }); });
    btnWrapper.appendChild(btn);

    const cal=document.createElement("button");
    cal.className="addcal-btn"; cal.textContent="+ Calendar";
    cal.addEventListener("click",e=>{ e.stopPropagation(); const dt=combineDateAndTime(dayDate,time); downloadICS({title:name,description:`${name} class`,start:dt,durationMinutes:60}); });
    btnWrapper.appendChild(cal);
  ...
  function render(){
    ...
    for(let i=0;i<7;i++){
      ...
      if(weekday===5){ addSession(sessions,"PUSH / CORE","06:00",day); addSession(sessions,"GRIP AND RIP (ERGS)","07:00",day); }
